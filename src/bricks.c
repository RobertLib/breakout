...
void initializeBricks(void)
{
  ...
  {
    if (
        LEVELS[paddle.level].pattern[i] != _ &&
        LEVELS[paddle.level].pattern[i] != E)
    {
      numBricks++;
    }
  }
  ...
  for (int i = 0; i < LEVEL_PATTERN_LENGTH; i++)
  {
    BrickType type = LEVELS[paddle.level].pattern[i];
  ...
  bricksTexture = loadTexture(assetsPath("bricks.bmp"));
  brickItemsTexture = loadTexture(assetsPath("brick-items.bmp"));
}
