...
static void drawBrickShadow(const Brick *brick)
{
  ...
  SDL_Rect srcRect = {0, 0, BRICK_WIDTH, BRICK_HEIGHT};
  ...
  SDL_RenderCopy(renderer, bricksTexture, &srcRect, &dstRect);
...
void drawBrick(const Brick *brick)
{
  ...
  SDL_RenderCopy(
      renderer,
      bricksTexture,
      &(SDL_Rect){brick->type * BRICK_WIDTH, 0, BRICK_WIDTH, BRICK_HEIGHT},
      &(SDL_Rect){
          brick->pos.x,
