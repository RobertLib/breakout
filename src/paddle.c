...
void paddleNextLevelUpdate(void)
{
  ...
  if (paddle.level < NUMBER_OF_LEVELS - 1)
  {
    reset();
...
void initializePaddle(void)
{
  ...
  paddle.texture = loadTexture(assetsPath("paddle.bmp"));
}
...
{
  if (paddle.inCollision)
  {
    SDL_SetTextureAlphaMod(paddle.texture, 255 / 2);
  }
  else if (paddle.type == PADDLE_TYPE_DYING)
  {
  ...
  SDL_RenderCopy(
      renderer,
      paddle.texture,
      &(SDL_Rect){paddle.type, 0, 1, PADDLE_HEIGHT},
      &(SDL_Rect){
          paddle.pos.x - paddleWidth() / 2,
